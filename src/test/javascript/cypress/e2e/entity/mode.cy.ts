...
describe('Mode e2e test', () => {
  ...
  const modeSample = { name: 'steeple', handle: 'debut' };
  ...
  describe('new Mode page', () => {
    ...
    it('should create an instance of Mode', () => {
      cy.get(`[data-cy="name"]`).type('obligation even now');
      cy.get(`[data-cy="name"]`).should('have.value', 'obligation even now');

      cy.get(`[data-cy="handle"]`).type('whether hamburger');
      cy.get(`[data-cy="handle"]`).should('have.value', 'whether hamburger');
